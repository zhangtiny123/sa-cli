...
const generateGridSelectionComponentCode = (tCols, tRows) => `
'use strict'
...
program.command('create')
  .option('-X, --cols <cols>', 'The number of columns in the grid', '16')
  .option('-Y, --rows <rows>', 'The number of rows in the grid', '8')
  .option('-d, --dir <dir>', 'The directory to save the component in', './src/components')
  .action((options) => {
    const content = generateGridSelectionComponentCode(options.cols, options.rows);
    const dirPath = options.dir;
    if (!fs.existsSync(dirPath)) {
      fs.mkdirSync(dirPath);
    }
    fs.writeFileSync(`${dirPath}/GridSelection.js`, content, 'utf8');
    console.log(`GridSelection component generated with ${options.cols} columns and ${options.rows} rows.`);
  });
